package com.example.trungcaosy.bat_pagoda.utils;

public class DataConstant {
    public static final String MQTT_SERVER = "tcp://m15.cloudmqtt.com:14517";
    public static final String MQTT_USERNAME = "tavncmmi";
    public static final String MQTT_PASSWORD = "ryl7wVmJgvzh";
    public static final String MQTT_CLIENT_ID = "trungcs";
    public static final String MQTT_BASE_TOPIC  = "chair/sensor/";

    public static final String OPEN_KEY = "OPEN";
    public static final int OPEN_CHAT = 2;
    public static final int OPEN_NEWS = 1;
    public static final int OPEN_SENSOR = 1;

    public static final String TOPIC_KEY = "topic";
//    public static final String BASE_URL = "https://lit-plateau-34257.herokuapp.com/";
    public static final String BASE_URL = "http://128.199.90.9:4000/";
    public static final String FCM_TOKEN_KEY = "FCM_TOKEN";
    public static final String APP_TOKEN_KEY = "APP_TOKEN";
    public static final String DEDAULT_TITLE = "Notice";;

    public static final String EMAIL_KEY = "EMAIL";
    public static final String PASSWORD_KEY = "PASSWORD";
    public static final String PROFILE_KEY = "PROFILE";

    public static final String NOTICE = "Thông báo";
    public static final String LOADING = "Đang tải";
}
